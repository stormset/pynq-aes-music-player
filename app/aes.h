...
public:
    ...
    void init();
    void destroy();
    void encrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string& input_path, const std::string& output_path,
                      const std::function<void(bool, void*)>* callback, void *callback_param);
    void encrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string& input_path, void *output_buffer, size_t output_buffer_size,
                      const std::function<void(bool, void*)>* callback, void *callback_param);
    void decrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string& input_path, const std::string& output_path,
                      const std::function<void(bool, void*)>* callback, void *callback_param);
    void decrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string& input_path, void *output_buffer, size_t output_buffer_size,
                      const std::function<void(bool, void*)>* callback, void *callback_param);
...
private:
    struct _dma_state {
        ...
        // details of the current ongoing transfer (if no ongoing transfer current_transfer.is_ongoing == false)
        struct {
            // is the transfer ongoing (this is used to indicate that the device is busy)
            ...
            // tx buffer <size, ptr>
            std::pair<size_t, void *> tx_buffer;
            // rx buffer <size, ptr>
            std::pair<size_t, void *> rx_buffer;
    ...
    _dma_state _cipher_state {.dev_index = CIPHER_DMA_INDEX, .dev{}, .tx_channel{}, .rx_channel{}, .dma_memories{}, .current_transfer{}, .state_mutex{}};
    _dma_state _decipher_state {.dev_index = DECIPHER_DMA_INDEX, .dev{}, .tx_channel{}, .rx_channel{}, .dma_memories{}, .current_transfer{}, .state_mutex{}};

    void *_aes_mem_ptr{};

    static void _do_transfer(uint32_t key[4], const std::string& input_path, const std::string& output_path, void *output_buffer, size_t output_buffer_size,
                      const std::function<void(bool, void*)>* callback, void *callback_param, _dma_state &dma_state);
