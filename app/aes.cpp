...
void aes::init() {
    _dma_state* dev_states[] = {&_cipher_state, &_decipher_state};
    ...
    for (auto &dev_state : dev_states) {
        ...
        dev_state->current_transfer.config_reg = (volatile uint32_t *)((char *)ptr + (dev_state == &_cipher_state ? AES_CIPHER_CFG_REG_OFFSET : AES_DECIPHER_CFG_REG_OFFSET));
...
void aes::encrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string& input_path,
                       const std::string& output_path, const std::function<void(bool, void *)> *callback, void *callback_param) {
    _do_transfer(key, input_path, output_path, nullptr, 0, callback, callback_param, _cipher_state);
}

void aes::encrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string &input_path, void *output_buffer,
                       size_t output_buffer_size, const std::function<void(bool, void *)> *callback, void *callback_param) {
    _do_transfer(key, input_path, "", output_buffer, output_buffer_size, callback, callback_param, _cipher_state);
}

void aes::decrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string &input_path,
                       const std::string &output_path, const std::function<void(bool, void *)> *callback, void *callback_param) {
    _do_transfer(key, input_path, output_path, nullptr, 0, callback, callback_param, _decipher_state);
}

void aes::decrypt_file(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string &input_path, void *output_buffer,
                       size_t output_buffer_size, const std::function<void(bool, void *)> *callback, void *callback_param) {
    _do_transfer(key, input_path, "", output_buffer, output_buffer_size, callback, callback_param, _decipher_state);
}

void aes::_dma_callback(int channel_id, void *data) {
    (void) channel_id;
    auto *dma_state = (aes::_dma_state *)data;
    const auto& rx_buff = dma_state->current_transfer.rx_buffer;
    const auto& tx_buff = dma_state->current_transfer.tx_buffer;
    ...
    if (!dma_state->current_transfer.output_file_path.empty()) {
        ...
        out_file.write((char *)rx_buff.second, (std::streamsize)rx_buff.first);
        out_file.close();
    } else if (dma_state->current_transfer.output_buffer) {
        memcpy(dma_state->current_transfer.output_buffer, rx_buff.second, rx_buff.first);
    }

    // cleanup
    pthread_mutex_lock(&dma_state->state_mutex);

    axidma_free(dma_state->dev, rx_buff.second, rx_buff.first);
    axidma_free(dma_state->dev, tx_buff.second, tx_buff.first);
    ...
    dma_state->dma_memories.erase(std::remove_if(
            ...
            [&rx_buff, &tx_buff](const std::pair<size_t, void *>& e) {
                return e.second == tx_buff.second || e.second == rx_buff.second; // put your condition here
            }), dma_state->dma_memories.end());
    dma_state->current_transfer.is_ongoing = false;
...
void aes::_do_transfer(uint32_t key[AES_KEY_WIDTH / sizeof(uint32_t)], const std::string &input_path,
                       ...
                       aes::_dma_state &dma_state) {
    void *rx_buff = nullptr;
    void *tx_buff = nullptr;
    std::streamsize aligned_file_size = 0;

    if (!dma_state.dev)
        throw std::runtime_error("DMA device is not initialied.");

    pthread_mutex_lock(&dma_state.state_mutex);

    try {
        if (dma_state.current_transfer.is_ongoing)
        ...
        // allocate tx buffer
        tx_buff = axidma_malloc(dma_state.dev, AES_KEY_WIDTH + aligned_file_size);
        if (!tx_buff)
            throw std::runtime_error("Unable to allocate continuous memory for transfer.");
        dma_state.current_transfer.tx_buffer = std::pair(AES_KEY_WIDTH + aligned_file_size, tx_buff);
        dma_state.dma_memories.emplace_back(dma_state.current_transfer.tx_buffer);

        // allocate rx buffer
        rx_buff = axidma_malloc(dma_state.dev, aligned_file_size);
        if (!rx_buff)
            throw std::runtime_error("Unable to allocate continuous memory for transfer.");
        dma_state.current_transfer.rx_buffer = std::pair(aligned_file_size, rx_buff);
        dma_state.dma_memories.emplace_back(dma_state.current_transfer.rx_buffer);
        ...
        *dma_state.current_transfer.config_reg = 0xFFFFFFFF;
        ...
        dma_state.current_transfer.is_ongoing = true;
        ...
        *dma_state.current_transfer.config_reg = 0x00000000;
        ...
        // start transfer
        int ret = axidma_twoway_transfer(dma_state.dev, dma_state.tx_channel, tx_buff, AES_KEY_WIDTH + aligned_file_size, nullptr,
                                         dma_state.rx_channel, rx_buff, aligned_file_size, nullptr, false);
        if (ret < 0)
            throw std::runtime_error("Failed to start transfer.");
    } catch(...) {
        // cleanup
        if (tx_buff)
            axidma_free(dma_state.dev, tx_buff, AES_KEY_WIDTH + aligned_file_size);
        if (rx_buff)
            axidma_free(dma_state.dev, rx_buff, aligned_file_size);
        if (rx_buff || tx_buff)
            dma_state.dma_memories.erase(std::remove_if(
                    dma_state.dma_memories.begin(), dma_state.dma_memories.end(),
                    [&rx_buff, &tx_buff](const std::pair<size_t, void *>& e) {
                        return e.second == tx_buff || e.second == rx_buff; // put your condition here
                    }), dma_state.dma_memories.end());
